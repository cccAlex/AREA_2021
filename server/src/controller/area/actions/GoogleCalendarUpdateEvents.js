...
const areaName = 'GoogleCalendarUpdateEvents';
...
const getDiff = (a1, a2) => a1.filter(elem1 => {
    for (const elem2 of a2) {
        if (elem2.id === elem1.id && elem2.updated != elem1.updated)
            return true;
    }
    return false;
});

const checkEvents = async (areaID, events, savecEvents) => {
    const updatedEvents = getDiff(events, savecEvents);

    if (updatedEvents.length) {
        await Area.updateOne({ name: areaName, areaID }, { params: JSON.stringify(events) }).exec();

        if (updatedEvents.length)
            return { status: true, data: updatedEvents };
    }
    return { status: false, data: null };
};

const GoogleCalendarUpdateEvents = async (areaID, {serviceParams}) => {
    try {
        ...
        console.log('GOOGLE CALENDAR UPATE EVENT ERROR:', err.message);
        return { status: false, data: null };
    }
};

module.exports = GoogleCalendarUpdateEvents;