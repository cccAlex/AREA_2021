...
class Youtube {
  ...
  static Future<http.Response> youtubeLogin(
      String state, String code, token) async {
    return await http.post(
      env['HOST'] + '/youtubeSignIn',
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': 'Bearer $token'
      },
  ...
  void youtubeOauth(var uri, context, token) async {
    var code, state, resp;
    if (uri.query.contains('code')) {
      ...
      if (code != null) {
        await Youtube.youtubeLogin(state, code, token);
